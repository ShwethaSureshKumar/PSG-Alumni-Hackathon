...
def nearest_neighbor(adjacency_matrix,ind,unvisited_cities):
...
def findmin(index,r0):
    ...
    for i in range(len(index)):
        if r0[index[i]] < min:
            min =r0[index[i]]
            ind=index[i]
    print("Minimum :", min, "at index",ind )
    return ind
...
while any(costList):
    if cap>costsum:
            if costList[i]+costsum>cap:
                   ...
                   costList[i]= costList[i]+costsum-cap
                   index.append(i)
                   print(costsum, "if if")
                   costsum=cap
                ...
                costsum+=costList[i]
                index.append(i)
                print(costsum, "if else")
                costList[i]=0
                i+=1
        
    elif costsum==cap:
         print("elif")
         startInd=findmin(index,r0)
         subtour=nearest_neighbor(adj_matrix,startInd,index)
         result_tour.append(subtour)
         index=[]
         costsum=0
...
totalcost=0
for tour in result_tour:
    stour=tour
    stour.pop()

    min_distance = calculate_tour_distance(stour, adj_matrix) + r0[stour[0]] + r0[stour[0]]
...
for i in range(len(result_tour)):
     for k in range(len(result_tour[i])):
          result_tour[i][k]="n"+str(result_tour[i][k]+1)
     result_tour[i].insert(0,"r0")
     result_tour[i].append("r0")
...
dict1={}
for i in range(len(result_tour)):
     dict1["path"+str(i+1)]=result_tour[i]
